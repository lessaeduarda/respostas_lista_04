...
dados_PNUD_pe <- dados_PNUD %>% filter(UF == 26, ANO == 2010
                                       )%>% select(Codmun7,IDHM
...
# Atribuir "Aluno_por_Docente" à variável "Mat_Doc"(número de matrículas / número de docentes) da base "dados_PNUD_pe":
Aluno_por_Docente <- dados_PNUD_pe$Mat_Doc

# Atribuir "IDHM" à variável "IDHM" da base "dados_PNUD_pe":
IDHM <- dados_PNUD_pe$IDHM
...
save(dados_PNUD_pe, file = "alunos_docentes_IDHM.RData")

### Questão 3: 

require(ggplot2)
plot(Aluno_por_Docente,IDHM)
